...
async def main():
    ...
    Your task:
    Connect to the MySQL database 'rahulshettyacademy'
    Query the 'RegistrationDetails' table to get a random record
    Query the 'Usernames' table to get additional user data
    Combine the data from both tables to create complete registration information
    Ensure the email is unique by adding a timestamp or random number if needed
    Prepare all the registration data in a structured format so that another agent can understand
    ...
    FIRST: Extract the EXACT registration data from DatabaseAgent's REGISTRATION_DATA message
    
    Read the Postman collection to understand the API contract
    
    Before making a registration API call- construct its body field with the DatabaseAgent's data inline with below format rules
    ...
    When login attempt is complete with successful registration api call, write: "API_TESTING_COMPLETE - ExcelAgent Should post
    ...
    Your task:
    Wait for APIAgent to complete with "API_TESTING_COMPLETE" message that include login call
    Extract the registration data from DatabaseAgent's REGISTRATION_DATA message
    Check APIAgent's response for actual login success/failure status
    Only save data if login was actually successful
    Open /Users/rahulshetty/files_claude/newdata.xlsx
    Add the registration data with current timestamp
    Save and verify the data
    CRITICAL: Only save data if APIAgent reports successful login, not just attempted login.
    When complete, write: "REGISTRATION_PROCESS COMPLETE" and stop.
    """))

    team = RoundRobinGroupChat(participants=[database_agent, api_agent,excel_agent],
                        termination_condition=TextMentionTermination("RIGISTRATION PROCESS COMPLETE"))
